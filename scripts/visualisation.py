...
def display_graph(graph: MultiDiGraph, colors_paths: dict[str, str], annotations: dict, output_path: str) -> None:
    ...
    #graph_visualizer.toggle_stabilization(False)
    graph_visualizer.set_edge_smooth('dynamic')
    #graph_visualizer.show_buttons(filter_=["physics"])
...
def filter_tsv(tsv_file:str, graph_a:MultiDiGraph, graph_b:MultiDiGraph) -> dict[list[str],str]:
    nodes_a:list[str] = [node[2:] for node in graph_a.nodes.keys()]
    nodes_b:list[str] = [node[2:] for node in graph_b.nodes.keys()]

    arrows_comparison:dict[list[str],str] = dict()

    with open(tsv_file,'r') as reader:
        for line in reader:
            if line.startswith('#'):
                continue
            path_name,position,operation,node_a,node_b,breakpoint_a,breakpoint_b = line.split('\t')
            if node_a in nodes_a and node_b in nodes_b:
                v_1,v_2 = arrows_comparison.get((node_a,node_b),(0,0))
                if operation == 'M':
                    v_1 += 1
                elif operation == 'S':
                    v_2 += 1
                arrows_comparison[(node_a,node_b)] = (v_1,v_2)
    return arrows_comparison
...
def multigraph_viewer(
    ...
    for node in pangenome_graph_A.nodes:
        pangenome_graph_A.nodes[node]['color'] = 'red'
    ...
    for node in pangenome_graph_B.nodes:
        pangenome_graph_B.nodes[node]['color'] = 'blue'

    edge_bank = filter_tsv(file_editions,pangenome_graph_A,pangenome_graph_B)
...
if __name__ == "__main__":
    file_a,file_b, edits_tsv, output_html, ref, start, stop = argv[1:]

    multigraph_viewer(
        file_A=file_a,
        file_B=file_b,
        file_editions=edits_tsv,
        output=output_html,
        reference=ref,
        start=int(start),
        end=int(stop),
